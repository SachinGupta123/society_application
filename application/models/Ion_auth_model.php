...
class Ion_auth_model extends CI_Model
{
	...
	public function register($identity, $password, $email, $additional_data = [], $groups = [])
	{
		...
		$query = $this->db->get_where($this->tables['roles'], ['slug' => $this->config->item('default_group', 'ion_auth')], 1)->row();
		...
		//add row password as discuss harsh sir -07-03-2022
	 ...
	 * @author    Mathew
	 */
	public function login($identity, $password, $remember=FALSE, $idCol="")
	{
		...
		if($idCol == ""){
			$idCol = $this->identity_column;
		}

		$query = $this->db->select($this->identity_column . ', username, email, phone ,id, password, active, last_login')
						  // ->where($this->identity_column, $identity)
						  ->where($idCol, $identity)
						  ->limit(1)
						  ->order_by('id', 'desc')
						  ->get($this->tables['users']);
		...
		if (isset($groups))
		{
			...
			if (isset($groups) && !empty($groups))
			{
				...
				$this->db->join(
				    ...
				    $this->tables['user_roles'].'.'.$this->join['users'].'='.$this->tables['users'].'.id',
				    'inner'
				);
			}
			...
			{
				$this->db->join($this->tables['roles'], $this->tables['user_roles'] . '.' . $this->join['roles'] . ' = ' . $this->tables['roles'] . '.id', 'inner');
				$this->db->where_in($this->tables['roles'] . '.name', $group_names);
			}
			if(!empty($group_ids))
			{
				$this->db->{$or_where_in}($this->tables['user_roles'].'.'.$this->join['roles'], $group_ids);
			}
		}
	...
	public function get_users_groups($id = FALSE)
	{
		...
		return $this->db->select('user_roles.role_id as id, roles.slug as name, roles.name as description')
	...
	public function add_to_group($group_ids, $user_id = FALSE)
	{
		...
		foreach ($group_ids as $group_id)
		{
			...
			if ($this->db->insert($this->tables['user_roles'],
								  [ 'role_id' => (float)$group_id,
	...
	public function remove_from_group($group_ids = FALSE, $user_id = FALSE)
	{
		...
		if (!empty($group_ids))
		{
			...
			foreach ($group_ids as $group_id)
			{
				...
				$this->db->delete(
					$this->tables['user_roles'],
					[$this->join['roles'] => (float)$group_id, $this->join['roles'] => (float)$user_id]
				);
		...
		{
			// Cast to float to support bigint data type
			if ($return = $this->db->delete($this->tables['user_roles'], [$this->join['users'] => (float)$user_id]))
			{
	...
	public function create_group($group_name = FALSE, $group_description = '', $additional_data = [])
	{
		...
		$data = ['name'=>$group_name,'slug'=>$group_description];
	...
	public function delete_group($group_id = FALSE)
	{
		...
		$this->db->delete($this->tables['user_roles'], [$this->join['roles'] => $group_id]);
