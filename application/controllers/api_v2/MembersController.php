...
class MembersController extends CI_Controller
{
    ...
    //     $data['flatcount'] = $this->db->get_where('members',array('user_id'=>$user_id))->num_rows();
    ...
    //     $bill_id = $params['bill_id'];
    //     $bill = $this->Bill_detail_model->get_bill_detail($bill_id);
    //     $data['bill_status'] = $bill['bill_status'];
    //     if(!empty($data)):
    //        echo json_encode($data);
    //         return;
    //     else:
    ...
    //         'return_url'=>'https://msociety.paynet.co.in/bill_detail/collect_response',
    ...
    public function society_details()
    {
        ...
        $user_id = $params['user_id'];
        $member_id = $params['member_id'];
        $mem = $this->Member_model->get_member_by_user_id($user_id,$member_id);
        if(empty($mem)):
            echo "User Does not Exists!";
            return;
        endif;
        $data['member'] = $mem[0];
        $soc = $this->Society_model->get_society_details($mem[0]->society_id);
        $data['society'] = $soc[0];
        if(!empty($data)):
           echo json_encode($data);
            return;
    ...
    public function get_all_society_by_member_id()
    {
        ...
        parse_str($url['query'], $params);
        $user_id = $params['user_id'];
        $society_by_member['society_by_member'] = $this->Member_model->get_society_name_by_member_id($service_provider_id);
    ...
    public function details()
    {
        ...
        $user_id = $params['user_id'];
        $member_id = $params['member_id'];
        $mem = $this->Member_model->get_member_by_user($user_id,$member_id);
        $data['flatcount'] = $this->db->get_where('members',array('user_id'=>$user_id,'id'=>$member_id))->num_rows();
        // $arrear = $this->Member_model->get_member_current_arrears($mem['id']);
        $arrear = $this->Member_model->get_member_current_balance($mem['id']);
        if(empty($mem)):
            echo "Member Does not Exists!";
    ...
    public function payment_history()
    {
        ...
        $member = $mem[0];
        $total_payment = 0;
        $payments = $this->Payment_model->get_payment_history_by_member_id($member_id);
        // print_r($payments);
        if(!empty($payments) && $payments != NULL):
            foreach($payments as $pay):
                $total_payment = $total_payment + $pay['debit'];
    ...
    public function payment_details()
    {
        ...
        if(empty($payment)):
            $resp = array(
                'response_code'=>'1001',
                'message'=>'Payment Details Does not Exists!',
            );
            echo json_encode($resp);
            return;
        endif;
    ...
    public function society_member_details()
    {
        ...
        $member_id = $params['member_id'];
        $mem = $this->Member_model->get_member_by_user_id($user_id,$member_id);
        $arrear = $this->Member_model->get_member_current_arrears($member_id);
        // print_r($arrear);die;
        if(empty($mem)):
            echo "Member Does not Exists!";
            return;
    ...
    public function bill_logs()
    {
        ...
        $member_id = $params['member_id'];
        $mem = $this->Member_model->get_member_by_user_id($user_id,$member_id);
        $bal = $this->Member_model->get_member_current_balance($member_id);
        if(empty($mem)):
            echo "Member Does not Exists!";
            return;
        endif;
        $data['member_balance'] = $bal;
        $data['bill_logs'] = $this->Bill_detail_model->get_member_bill_logs($member_id);
        if(!empty($data)):
           echo json_encode($data);
            return;
        else:
    ...
    public function bill_details()
    {
        ...
        $credit_notes = $this->Payment_model->get_credit_note_bill($member['id'], $last_month, $b_d);
        $data['bill'] = $bill;
        $billDetails = unserialize($bill['details']);
        $newBillDetails = array();
        foreach($billDetails as $key => $val){
            $newKey = str_replace(' ', '_', $key);
            $newBillDetails[$newKey] = $val;
        }
        $data['bill']['details'] = $newBillDetails;
        if(!empty($credit_notes)):
            $data['credit_notes'] = $credit_notes;
        else:
            $data['credit_notes'] = false;
    ...
    public function bill_logs_current()
    {
        ...
        $bill_details = $this->Bill_detail_model->get_last_bill_details_member($member_id);
        $data['bill_details'] = $bill_details[0];
        $data['bill_details']['details'] = unserialize($bill_details[0]['details']);
        $data['credit_notes'] = $this->Payment_model->get_credit_notes_member($member_id);
        $data['debit_notes'] = $this->Payment_model->get_debit_notes_member($member_id);
        $data['member_balance'] = $bal;
        $data['member_previous_balance'] = $this->Member_model->get_member_previous_balance($member_id);
        if(!empty($data)):
           echo json_encode($data);
            return;
        else:
    ...
    public function bill_logs_unpaid()
    {
        ...
        $unpaid_bills = $this->Bill_detail_model->get_all_bill_unpaid_by_member($member_id);
        $bill_details = $this->Bill_detail_model->get_all_bill_by_member($member_id);
        $data['unpaid_bills'] = $unpaid_bills;
        // print_r($data['unpaid_bills']);die;
        foreach ($data['unpaid_bills'] as $index => $rows) {
            $data['unpaid_bills'][$index]['details'] = unserialize($unpaid_bills[$index]['details']);
    ...
    public function generatePaypage()
    {
        ...
        $soc = $this->Society_model->get_society_details($member['society_id']);
        $society = $soc[0];
        $flat_owner = $this->Member_model->get_flat_owner($bill['member_id']);
        // print_r($flat_owner);return;
        // $user = $this->User_model->get_user($flat_owner['user_id']);
        // echo json_encode(array("paypage"=>array("bill_id"=>$bill_id)));
        ...
        $post = array(
            ...
            'unit_price'=>$bill['total_due'],
            'reference_no'=>$bill_id,
            'title'=>preg_replace("/[^ \w]+/", "", $flat_owner['first_name']),
            'site_url'=>'https://msociety.paynet.co.in',
            'products_per_title'=>'Monthly Rental',
            'email'=>$flat_owner['email'] ? $flat_owner['email'] : "NULL",
            'return_url'=>'https://msociety.paynet.co.in/bill_detail/collect_response',
            'm_first_name'=>preg_replace("/[^ \w]+/", "", $flat_owner['first_name']),
            'm_last_name'=>preg_replace("/[^ \w]+/", "", $flat_owner['last_name']),
            'm_phone_number'=>$flat_owner['phone'],
            'phone_number'=>$flat_owner['phone'],
            'billing_address'=>$society->address,
            'city'=>'Vasai',
            'state'=>'Maharashtra',
            'postal_code'=>'401201',
    ...
    public function generatePaypageP()
    {
        $url = parse_url($_SERVER['REQUEST_URI']);
        parse_str($url['query'], $params);
        $bill_id = $params['bill_id'];
        ...
        // $this->Bill_detail_model->update_bill_detail($bill_id, $url1);
        if($url1['pay_url'] != '' || !empty($url1['pay_url'])):
           echo json_encode($url1);
            return;
        else:
            $resp = array(
    ...
    //     $member = $this->Member_model->get_member($member_id);
    //     $data['user'] = $this->User_model->get_user($member['user_id']);
    ...
    public function get_member_info()
    {
        ...
        $member_id = $request['user']['member_id'];
        $user = $this->User_model->get_user($user_id);
        $member = $this->Member_model->get_member_by_user($user_id,$member_id);
        // echo json_encode($member);return;
        $society_id = $member['society_id'];
        $society = $this->Society_model->get_society($society_id);
        $members = array(
    ...
    public function verifyPayment(){
        $data = $_REQUEST;
        $resp = array('status'=>1,'message'=>'Payment Initiated');
        echo json_encode($resp);
        return 1;
    }
}
