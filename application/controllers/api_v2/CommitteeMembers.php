...
class CommitteeMembers extends CI_Controller
{
    ...
    public function payment_history()
    {
        ...
        $member = $mem[0];
        $total_payment = 0;
        $payment = $this->Payment_model->get_payment_by_member_id($member_id);
        if(!empty($payment)):
        foreach($payment as $pay):
            $total_payment = $total_payment + $pay['credit'];
        endforeach;
    ...
    public function payment_list()
    {
        ...
        $member = $mem[0];
        $pay = $this->Payment_model->get_payment_list_by_member_id($member_id);
        $payment['payment'] = $pay[0];
        if(!empty($payment)):
            echo json_encode($payment);
            return;
    ...
    public function member_payments_list()
    {
        ...
        if(!empty($all_payments)):
            ...
            $data['all_payments'] = $all_payments;
            $data['all_members'] = $all;
            $data['total_payment'] = $this->Payment_model->get_total_revenue_by_society_id($society->id);;
            echo json_encode($data);
            return;
        else:
            $resp = array(
    ...
    public function committee_member_billing_heads()
    {
        ...
        if(empty($b_heads)):
            $resp = array(
                'response_code'=>'1001',
                'message'=>'Bill Head Does not Exist',
            );
            echo json_encode($resp);
            return;
        endif;
    ...
    //     $data['all_members'] = $all;
    //     $bill_details = $this->Society_model->get_all_bill_by_user($society->id);
    //     $unpaid_bills = $this->Society_model->get_all_bill_unpaid_by_user($society->id);
    //     $total_pending = 0;
    //     $data['unpaid_bills'] = $unpaid_bills;
    //     // print_r($unpaid_bills);die;return;
    //     foreach ($unpaid_bills as $key => $bills)
    //     {
    //         $data['unpaid_bills'][$key]['details'] = unserialize($bills['details']);
    ...
    //         }
    //         $data['unpaid_bills'][$key]['details'] = $newBillDetails;
    //     }
    //     $data['credit_notes'] = $this->Payment_model->get_credit_notes($society->id);
    //     $data['debit_notes'] = $this->Payment_model->get_debit_notes($society->id);
    //     $data['total_pending'] = $this->Member_model->get_total_outstanding_society($society->id);
    ...
    public function society_pending_amount()
    {
        ...
        $data['all_members'] = $all;
        $bill_details = $this->Society_model->get_all_bill_by_user($society->id);
        $data['unpaid_bills'] = $unpaid_bills = $this->Society_model->get_all_bill_unpaid_by_user($society->id);
        $total_pending = 0;
        $data['unpaid_bills'] = $unpaid_bills;
        // print_r($unpaid_bills);die;return;
        foreach ($unpaid_bills as $key => $bills)
    ...
    public function society_bank_transactions()
    {
        $url = parse_url($_SERVER['REQUEST_URI']);
        parse_str($url['query'], $params);
        $society_id = $params['society_id'];
        ...
        $bank_transactions = $this->Bank_transaction_model->get_all_bank_transactions($bank_id, $society_id);
        if(empty($bank_transactions)):
            $resp = array(
                'response_code'=>'1002',
                'message'=>'Bank Transactions not Found',
                'bank_transactions'=>'',
            );
            echo json_encode($resp);
            return;
